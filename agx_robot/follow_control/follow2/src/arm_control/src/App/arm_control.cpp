...
void arx_arm::update_real(ecat::EcatBase ecat_base)
{
    ...
    else
    {
        if(control_mode == 0  ||  control_mode == 1 ||control_mode==4 || control_mode==5 )//通过解算控制关节
        {
            ...
            is_starting=0;
            motor_control(ecat_base);;ROS_INFO("pos_control>>>>>>>>>");
            gripper_control(ecat_base);
        }

        if(control_mode == 2  ||  control_mode == 3)//直接控制关节
        {
            joint_control(ecat_base);ROS_INFO("joint_control>>>>>>>>>");
        }
...
void arx_arm::joint_control(ecat::EcatBase ecat_base)
{
    ...
    if(current_normal)
    {
        ...
        limit_joint(ros_control_pos);
            ...
            CAN_Handlej.Send_moto_Cmd1(ecat_base, 1, 150, 12, ros_control_pos[0], 0,  solve.joint_torque[0]);
            CAN_Handlej.Send_moto_Cmd1(ecat_base, 2, 210, 12, ros_control_pos[1], 0,  solve.joint_torque[1]);
            CAN_Handlej.Send_moto_Cmd1(ecat_base, 4, 150, 12, ros_control_pos[2], 0,  solve.joint_torque[2]);
            CAN_Handlej.Send_moto_Cmd2(ecat_base, 5, 30, 0.8, ros_control_pos[3], 0,  solve.joint_torque[3]);
            CAN_Handlej.Send_moto_Cmd2(ecat_base, 6, 25, 0.8, ros_control_pos[4], 0,  solve.joint_torque[4]);
            CAN_Handlej.Send_moto_Cmd2(ecat_base, 7, 10, 1  , ros_control_pos[5], 0,  solve.joint_torque[5]); 
            CAN_Handlej.Send_moto_Cmd2(ecat_base, 8, 4, 1   , ros_control_pos_t[6], 0,  0); 
...
void arx_arm::init_step(ecat::EcatBase ecat_base)
{
    solve.init_pos(target_pos,current_pos,target_pos_temp,is_starting,is_arrived ,teach2pos_returning,temp_init);
        ...
        CAN_Handlej.Send_moto_Cmd2(ecat_base, 6, 25, 0.8, target_pos_temp[4], 0,   solve.joint_torque[4] );
        CAN_Handlej.Send_moto_Cmd2(ecat_base, 7, 10, 1,   target_pos_temp[5], 0,   solve.joint_torque[5] ); 
        CAN_Handlej.Send_moto_Cmd2(ecat_base, 8, 3, 1, 0, 0, 0 );
        ecat_base.EcatSyncMsg(); CAN_Handlej.can0_ReceiveFrame(ecat_base); 
    cmd_init();
    is_teach_mode = false;
...
void arx_arm::gripper_control(ecat::EcatBase ecat_base)
{
            ...
            CAN_Handlej.Send_moto_Cmd2(ecat_base,8, 3, 1,   gripper_pos, 0,  0 );
            ecat_base.EcatSyncMsg(); CAN_Handlej.can0_ReceiveFrame(ecat_base); 
}
